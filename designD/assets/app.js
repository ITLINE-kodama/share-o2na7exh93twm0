...
const io2=new IntersectionObserver(es=>es.forEach(e=>{
  ...
  const el=e.target,n=parseFloat(el.dataset.n),from=parseFloat(el.textContent)||0,dur=1400,st=performance.now();
  // JS無効・スクリーンリーダーでも実数が読めるよう、初期DOM値(=実数)から開始し0に戻さない
  (function f(now){const p=Math.min((now-st)/dur,1),v=from+(n-from)*(1-Math.pow(1-p,3));
...
// メガナビ: ＋クリックで下層を右カラムに表示（LINEヤフー型）
(function(){
  document.querySelectorAll('.mega .mg-tgl').forEach(function(btn){
    btn.addEventListener('click', function(e){
      ...
      mega.querySelectorAll('.mg-item.open').forEach(function(o){
        o.classList.remove('open');
        var t=o.querySelector('.mg-tgl'); if(t) t.textContent='＋';
      });
      mega.querySelectorAll('.mg-sub.show').forEach(function(s){ s.classList.remove('show'); });
      if(!wasOpen && panel){ item.classList.add('open'); btn.textContent='−'; panel.classList.add('show'); }
    });
  });
})();
